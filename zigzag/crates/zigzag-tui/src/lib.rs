/// zigzag-tui: ratatui-based TUI frontend for Zigzag.
///
...
use zigzag_core::action::{ActionDef, ActionType, PaneType, ResolvedAction};
use zigzag_core::config::SwitcherPriorityCriterion;
...
use zigzag_core::theme::{Rgb, ThemeStyle};
use zigzag_core::traits::SessionRefresher;
...
pub struct ProjectEntry {
    ...
    /// Per-repo action definitions (from `.config/zigzag.kdl`).
...
pub struct GitInfo {
    ...
    pub review: Option<zigzag_core::domain::ReviewStatus>,
}
...
impl zigzag_core::traits::ForgeClient for NoopForgeClient {
    fn get_pr(&self, _: &str, _: &str) -> zigzag_core::error::Result<Option<PullRequest>> {
        Ok(None)
    }

    fn get_ci_status(&self, _: &str, _: &str) -> zigzag_core::error::Result<CiStatus> {
        Ok(CiStatus::Unknown)
    }
    ...
    fn get_review_status(
        ...
        _: &str,
        _: &str,
    ) -> zigzag_core::error::Result<Option<zigzag_core::domain::ReviewStatus>> {
        Ok(None)
    }
}
...
pub struct TuiState {
    ...
    pub forge_client: Arc<dyn zigzag_core::traits::ForgeClient + Send + Sync>,
    ...
    pub theme: zigzag_core::theme::Theme,
    ...
    /// Global action definitions (from `~/.config/zigzag/config.kdl`).
...
impl TuiState {
    ...
    pub fn new(
        ...
        forge_client: Arc<dyn zigzag_core::traits::ForgeClient + Send + Sync>,
        ...
        Self {
            ...
            modal: None,
            status_message: None,
            theme: zigzag_core::theme::Theme::default(),
            leader_pending: None,
    ...
    pub fn trigger_preview_load(&mut self) {
        ...
        let session_name = if branch.is_empty() {
            ...
            format!(
                "{}:{}",
                project_name,
                zigzag_core::domain::sanitize_branch_name(&branch)
            )
        };
        ...
        if let Some(json) = json {
            ...
            }) = &mut self.modal
            {
                let parsed = match kind {
                    GhPickerKind::Issue => zigzag_core::gh::parse_gh_issues(&json),
                    GhPickerKind::Pr => zigzag_core::gh::parse_gh_prs(&json),
                };
...
fn parse_zellij_json_for_session(json: &str, session_name: &str) -> Option<ZellijInfo> {
    let info = zigzag_core::zellij::parse_zellij_session_info(json, session_name)?;
    Some(ZellijInfo {
            ...
            if state.modal.is_some() {
                ...
                match outcome {
                    ...
                    ModalOutcome::GhIssueSelected {
                        ...
                        state.modal = None;
                        let slug = zigzag_core::domain::slugify(&title);
                    ...
                    KeyCode::Char('E') if state.focused_panel == Panel::Projects => {
                        if let Some(entry) = state.selected_entry() {
                            ...
                            form.fields[3].value = match &project.transport {
                                Some(zigzag_core::domain::Transport::Mosh) => "mosh".to_string(),
                                Some(zigzag_core::domain::Transport::Ssh) => "ssh".to_string(),
                                None => String::new(),
                            };
...
fn build_action_menu(state: &TuiState) -> Vec<ResolvedAction> {
    use zigzag_core::action::{self, ActionEnv, ActionPreview};
...
fn worktree_notification_alias(wt: &WorktreeEntry) -> Option<String> {
    wt.discovered
        .branch
        .as_deref()
        .map(|branch| zigzag_core::domain::derive_session_name(&wt.discovered.project_name, branch))
}
...
fn render_status(f: &mut Frame, area: Rect, state: &TuiState) {
    ...
    let first_line = if let Some(msg) = &state.status_message {
        ...
        state
            ...
            .unwrap_or_else(|| " No projects — add to ~/.config/zigzag/projects.kdl ".to_string())
    };
...
fn render_doctor_modal(f: &mut Frame, diagnostics: &[String], theme: &zigzag_core::theme::Theme) {
    let area = f.area();
...
fn render_help_modal(f: &mut Frame, theme: &zigzag_core::theme::Theme) {
    let area = f.area();
...
/// State for the `zigzag switch` session picker TUI.
#[derive(Debug, Clone)]
pub struct SwitchPickerState {
    /// All Zigzag-managed session names, sorted alphabetically.
...
pub fn sort_switch_entries(
    ...
    let order = if priority.is_empty() {
        zigzag_core::config::default_switcher_priority()
    } else {
        priority.to_vec()
    };
...
fn switch_notification_style(level: NotifyLevel, theme: &zigzag_core::theme::Theme) -> Style {
    match level {
...
mod tests {
    ...
    impl zigzag_core::traits::ForgeClient for MockForgeClient {
        fn get_pr(&self, _: &str, _: &str) -> zigzag_core::error::Result<Option<PullRequest>> {
            Ok(None)
        }
        fn get_ci_status(&self, _: &str, _: &str) -> zigzag_core::error::Result<CiStatus> {
    ...
    fn mock_forge() -> Arc<dyn zigzag_core::traits::ForgeClient + Send + Sync> {
    ...
    impl SessionRefresher for MockSessionRefresher {
        ...
        fn fetch_activity(&self) -> zigzag_core::activity::SessionActivity {
                ...
                identity: zigzag_core::domain::WorktreeIdentity {
                    ...
                    worktree_path: if branch == "main" {
                        ...
                        project
                            .path
                            .join(".worktrees")
                            .join(zigzag_core::domain::sanitize_branch_name(branch))
                    },
                },
    ...
    fn renders_review_new_comments() {
        ...
        let mut info = make_git_info();
        info.review = Some(zigzag_core::domain::ReviewStatus {
    ...
    fn complete_path_returns_matching_dirs_only() {
        ...
        let base = tmp.join("zigzag_tui_test_complete_path");
    ...
    fn complete_path_filters_by_prefix() {
        let tmp = std::env::temp_dir();
        let base = tmp.join("zigzag_tui_test_prefix_filter");
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn tab_on_path_field_single_match_completes_inline() {
        let tmp = std::env::temp_dir();
        let base = tmp.join("zigzag_tui_test_tab_single");
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn tab_on_path_field_multiple_matches_completes_to_prefix() {
        let tmp = std::env::temp_dir();
        let base = tmp.join("zigzag_tui_test_tab_multi");
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(base.join("foobar")).unwrap();
    ...
    fn edit_modal_tab_on_path_field_single_match_completes_inline() {
        let tmp = std::env::temp_dir();
        let base = tmp.join("zigzag_tui_test_edit_tab_single");
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(base.join("myproject")).unwrap();
    ...
    fn tab_on_path_field_single_match_autofills_name_if_empty() {
        let tmp = std::env::temp_dir();
        let base = tmp.join("zigzag_tui_test_tab_autofill");
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn switch_picker_sort_prioritizes_waiting_before_recent_by_default() {
        ...
        sort_switch_entries(
            &mut entries,
            &zigzag_core::config::SwitcherConfig::default().priority,
        );
    ...
    fn switch_picker_sort_with_waiting_priority_waiting_still_outranks_recent() {
        ...
        let priority = vec![
            zigzag_core::config::SwitcherPriorityCriterion::Waiting,
            zigzag_core::config::SwitcherPriorityCriterion::Recent,
        ];
