...
use zigzag_core::config::parse_autopilot_config_doc;
pub use zigzag_core::config::AutopilotConfig;
use zigzag_core::error::{Result, ZError};

/// Full autopilot configuration from `.config/zigzag.kdl`: project-level settings +
...
/// the contents of a per-repo `.config/zigzag.kdl` file.
