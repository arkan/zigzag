...
Provide an HTTP/WebSocket server interface for the Zigzag platform, exposing core
capabilities (activity queries, layout management, Claude hook endpoints,
session orchestration) over the network. Planned as phase 5 of the Zigzag roadmap,
...
edge to `zigzag-core`, and document the intended architecture.
...
- **Axum-based HTTP server** (declared intent): Axum's tower-based middleware
  ...
  existing `zigzag-core` error and trait idioms, enabling composable route modules.
...
- **Stub-first incremental delivery**: Introduced as an empty library with a
  single dependency on `zigzag-core`, serving as a compile-time scaffold.
...
- **Leaf-node dependency**: Eighth crate in the workspace member list, depends
  only on `zigzag-core`. No other crate depends on `zigzag-web` — it is a terminal
...
1. **Startup**: `zigzag-web` is launched (standalone or embedded via `zigzag-cli`),
   initializing an axum `Router` with shared `zigzag-core` state (config, activity
   store, layout engine).
2. **Inbound HTTP/WS**: External clients connect via REST endpoints or
   ...
   delegate to `zigzag-core` services for domain logic.
...
- **zigzag-core** (only dependency): Consumes config, activity, layout, error,
  and session types. Every handler and WS message translates between HTTP
  protocol and `zigzag-core` domain models. The `error` module's `thiserror`-
  derived types map to HTTP status codes via axum's `IntoResponse`.
- **zigzag-cli** (planned consumer): The CLI binary may embed or spawn the web
  server as a subcommand (`zigzag web serve`), passing the shared runtime handle.
...
- **Workspace position**: Terminal/leaf node — depends only on `zigzag-core` and
