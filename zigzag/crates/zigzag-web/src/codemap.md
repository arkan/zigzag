...
edge to `zigzag-core`, and document the intended architecture before active
development begins.

## Design

- **Stub-first incremental delivery**: The crate is introduced as an empty
  library with a single dependency on `zigzag-core`, serving as a compile-time
...
- **Axum as HTTP framework** (declared intent): The comment in `lib.rs`
  ...
  existing `zigzag-core` error and trait idioms, enabling composable route
  modules.
...
1. **Startup**: `zigzag-web` is launched (either standalone or embedded via
   `zigzag-cli`), initializing axum `Router` with shared `zigzag-core` state (config,
...
- **zigzag-core** (direct dependency): Consumes config, activity, layout, error,
...
- **Workspace position**: Eighth crate (last in member list), depends only on
  `zigzag-core`. It is a terminal/leaf node in the dependency graph — no other
  crate depends on `zigzag-web`.
