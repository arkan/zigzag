...
The `zigzag-cli` crate is the **CLI binary entry point** for the `zigzag` project manager. Its function is to mediate between the user (terminal/TUI) and the Zellij multiplexer, Git worktree infrastructure, and forge (GitHub) APIs. It implements all CLI subcommands (`open`, `close`, `delete`, `prune`, `notify`, `list`, `switch`, `logs`, `logs-viewer`, `actions`, `autopilot`) and the interactive TUI loop. It owns the concrete adapters for trait abstractions defined in `zigzag_core` — process-backed I/O, file-backed persistence, and SSH-encapsulated remote execution.
...
All domain interfaces are defined as traits in `zigzag_core::traits` (ports). The CLI crate provides concrete, process-backed implementations (adapters):
...
Project registration lives in `~/.config/zigzag/projects.kdl` (parsed by `zigzag_core::config::parse_projects_kdl`). Per-repo config lives in `<project>/.config/zigzag.kdl`, parsed by `parse_repo_config_projection` which uses two-phase parsing: first raw KDL document, then projections via `parse_per_repo_config_doc` (for `PerRepoConfig`) and `parse_autopilot_workflows_doc` (for `Vec<AutopilotWorkflow>`). Global config lives in `~/.config/zigzag/config.kdl` (parsed by `zigzag_core::config::parse_global_config_kdl`). `repo_config::RepoConfigProjection` bundles the relevant slices for CLI consumers.
...
2. `loop`:
   ...
   c. Calls `zigzag_tui::run_tui(...)` with a `TuiCallbacks` closure bundle.
...
2. If remote (has `host`): delegate to `cmd_open_remote` → SSH `zigzag open <project> <branch>` on host via `ssh -t` or `mosh`.
...
1. `zigzag switch` guard: must run inside a Zellij session (`ZELLIJ_SESSION_NAME` set). Dashboard switcher loads entries for an in-dashboard modal, accepts no current session, and attaches interactively from outside Zellij after the TUI exits.
2. Global lock file (`/tmp/zigzag-switch.lock`) prevents concurrent invocations (cleanup via `LockGuard` Drop impl).
3. List all Zigzag-managed sessions with ages and notification counts → sort by recent activity → standalone `run_switch_picker` for `zigzag switch`, or `Modal::SwitchPicker` for the dashboard.
...
2. Loads global + per-repo actions, merges with built-in actions via `zigzag_core::action::merge_actions`.
...
3. `FileRunStore`: delegates to `zigzag_autopilot::persist::{load_run, save_run, delete_run}`.
...
2. Metadata-backed notifications are stored in `~/.config/zigzag/worktree-metadata.json` via `LocalWorktreeMetadataStore::add_notification`.
