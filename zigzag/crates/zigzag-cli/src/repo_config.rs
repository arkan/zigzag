use zigzag_autopilot::dsl::{parse_autopilot_workflows_doc, AutopilotWorkflow};
use zigzag_core::config::{parse_per_repo_config_doc, PerRepoConfig};
use zigzag_core::error::{Result, ZError};

/// All CLI projections from a repo-local `.config/zigzag.kdl` file.
...
/// Parse `.config/zigzag.kdl` once and project the slices needed by CLI/TUI callers.
