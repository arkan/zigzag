...
use zigzag_core::activity::SessionActivity;
use zigzag_core::domain::{DiscoveredWorktree, Layout, Project, Session};
use zigzag_core::error::{Result, ZError};
use zigzag_core::traits::{SessionManager, SessionRefresher, WorktreeMetadataStore};
...
pub struct ZellijSessionManager {
    /// Absolute path to the `zigzag` binary, used in generated Zellij keybinds.
    pub bin_path: String,
}
...
impl SessionManager for ZellijSessionManager {
    ...
    fn create_session(
        ...
        let kdl = zigzag_core::layout::generate_layout_kdl(&layout, &self.bin_path, theme);
...
/// Parse `zellij list-sessions` output and return active Zigzag-managed sessions with their ages.
///
...
/// List all active Zigzag-managed sessions with their ages.
pub fn list_all_zigzag_sessions_with_ages() -> Vec<(String, Option<String>)> {
    ...
    let raw = String::from_utf8_lossy(&output.stdout);
    let stdout = strip_ansi(&raw);
    list_all_zigzag_sessions_with_ages_from_output(&stdout)
}
...
mod tests {
    ...
    // --- list_all_zigzag_sessions_with_ages_from_output tests ---
    ...
    fn sessions_with_ages_empty_output() {
        let sessions = list_all_zigzag_sessions_with_ages_from_output("");
        assert!(sessions.is_empty());
    }
