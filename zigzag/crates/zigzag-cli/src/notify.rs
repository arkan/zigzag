/// Notifier implementations for zigzag-cli.
///
...
use zigzag_core::config::NotificationsConfig;
...
impl Notifier for MacosNotifier {
    fn notify(&self, message: &str, level: NotifyLevel) -> Result<()> {
        let title = match level {
            NotifyLevel::Info => "Zigzag",
            NotifyLevel::Warning => "Zigzag ⚠️",
            NotifyLevel::Error => "Zigzag ❌",
        };
        let script = format!(
