...
use zigzag_core::config::{effective_layout, parse_global_config_kdl, GlobalConfig, PerRepoConfig};
use zigzag_core::depcheck::{check_deps, format_dep_error, DepCheckStatus};
use zigzag_core::domain::{NotifyLevel, Session};
use zigzag_core::log::{LogLevel, Logger};
use zigzag_core::traits::{
...
use crate::session_manager::{
    list_all_zigzag_sessions_with_ages, parse_session_name, ZellijSessionManager,
    ZellijSessionRefresher,
};
use crate::worktree_manager::WtWorktreeManager;

use zigzag_tui::{Navigation, PreviewContext, PreviewDataSource, ProjectEntry, TuiAction};
...
fn resolve_required_session_env(command: &str) -> zigzag_core::error::Result<String> {
    resolve_session_env()
        .filter(|session| !session.is_empty())
        .ok_or_else(|| {
            zigzag_core::error::ZError::Session(format!(
                "zigzag {command} must be run inside a Zellij session \
             (neither ZIGZAG_SESSION_NAME nor ZELLIJ_SESSION_NAME is set)"
            ))
        })
}
...
fn main() {
    ...
    if failed && !is_doctor {
        eprintln!("\nZigzag requires all dependencies to be installed. Aborting.");
        std::process::exit(1);
    }
...
fn run() {
    ...
    match args.first().map(|s| s.as_str()) {
        ...
        Some("open") => {
            ...
            if project.is_empty() {
                eprintln!("usage: zigzag open <project> [branch]");
                std::process::exit(1);
            }
            ...
            match sub {
                "kill" => {
                    ...
                    if project.is_empty() || branch.is_empty() {
                        eprintln!("usage: zigzag session kill <project> <branch>");
                        std::process::exit(1);
                    }
        ...
        Some("project") => {
            ...
            match sub {
                "delete" => {
                    let project = args.get(2).map(|s| s.as_str()).unwrap_or("");
                    if project.is_empty() {
                        eprintln!("usage: zigzag project delete <project>");
                        std::process::exit(1);
                    }
...
fn cmd_tui() -> zigzag_core::error::Result<()> {
    ...
    fn build_entries(
        ...
        for project in &projects {
            ...
            let safety_map: HashMap<WorktreeIdentity, zigzag_core::domain::GitSafetyStatus> =
            ...
            let active_sessions: Vec<Session> = worktree_entries
                ...
                .filter_map(|wt| match &wt.session_link {
                    zigzag_core::domain::SessionLink::Active(s) => Some(s.clone()),
                    _ => None,
                })
                .collect();

            let repo_config_path = project.path.join(".config").join("zigzag.kdl");
    ...
    loop {
        ...
        let theme = zigzag_core::theme::Theme::from_name(global.theme);

        let callbacks = zigzag_tui::TuiCallbacks {
            log_fn: &|max_lines| {
            ...
            kill_session_fn: &|session_name| {
                ...
                let sess = zigzag_core::domain::Session {
            ...
            add_project_fn: &|path, name, host, transport| {
                let transport = match transport {
                    Some("mosh") => Some(zigzag_core::domain::Transport::Mosh),
                    Some("ssh") => Some(zigzag_core::domain::Transport::Ssh),
                    _ => None,
                };
                let project = zigzag_core::domain::Project {
                    name: name.to_string(),
            ...
            load_switch_entries_fn: &|| {
                ...
                Ok(zigzag_tui::SwitchEntriesSnapshot {
                    entries,
        ...
        match action {
            ...
            TuiAction::NewFromIssue {
                ...
                let prompt = zigzag_core::template::resolve_template(&template, &vars);
            ...
            TuiAction::RunAction {
                ...
                let is_tab = matches!(request.pane_type, zigzag_core::action::PaneType::Tab);
...
fn discover_local_worktrees_for_projects(
    projects: &[zigzag_core::domain::Project],
) -> Vec<zigzag_core::domain::DiscoveredWorktree> {
    projects
        .iter()
...
fn migrate_local_metadata_if_needed() -> zigzag_core::error::Result<()> {
    let store = KdlProjectStore::new();
    let projects = store.list_projects()?;
...
fn load_per_repo_config(project_path: &std::path::Path) -> PerRepoConfig {
    let path = paths::repo_config_path(project_path);
    match fs::read_to_string(&path) {
...
fn cmd_edit_per_repo_config(project_path: &std::path::Path) -> zigzag_core::error::Result<()> {
    let config_dir = project_path.join(".config");
    let config_file = paths::repo_config_path(project_path);

    // Create .config/ directory if missing (create_dir_all is a no-op if it exists).
    fs::create_dir_all(&config_dir).map_err(|e| zigzag_core::error::ZError::Io(e.to_string()))?;

    // Create the file with a commented template if it doesn't exist.
    if !config_file.exists() {
        let template = "\
// Per-repo Zigzag configuration
...
";
    ...
    let status = std::process::Command::new(&editor)
        ...
        .status()
        .map_err(|e| {
            zigzag_core::error::ZError::Io(format!("failed to launch editor '{}': {}", editor, e))
        })?;
...
fn cmd_open(
    ...
    if let Some(prompt_text) = prompt {
        zigzag_core::layout::inject_prompt_into_layout(&mut layout, prompt_text);
    }
...
fn record_worktree_entry(
    project: &zigzag_core::domain::Project,
    branch: &str,
    path: &std::path::Path,
    session_name: &str,
) -> zigzag_core::error::Result<()> {
    let discovered = zigzag_core::domain::DiscoveredWorktree {
        identity: zigzag_core::domain::WorktreeIdentity {
            host: project.host.clone(),
            project_root: project.path.clone(),
    ...
    zigzag_core::session_entry::record_session_attach(
...
/// `zigzag open <project> <branch>` there. Zellij runs on the remote machine.
///
...
fn cmd_open_remote(
    ...
    .map_err(|e| zigzag_core::error::ZError::Session(e.to_string()))?;

    if !status.success() {
        let transport = if use_mosh { "mosh" } else { "ssh" };
        return Err(zigzag_core::error::ZError::Session(format!(
...
///
/// `session_name` — if `None`, detects the current session from `ZIGZAG_SESSION_NAME`
/// with `ZELLIJ_SESSION_NAME` fallback.
fn cmd_close(session_name: Option<&str>) -> zigzag_core::error::Result<()> {
    let session_mgr = ZellijSessionManager {
        bin_path: resolve_bin_path(),
    };
...
fn delete_project_from_store(
    ...
    project_name: &str,
) -> zigzag_core::error::Result<String> {
    let name = project_name.trim();
    if name.is_empty() {
        return Err(zigzag_core::error::ZError::ConfigParse(
...
fn cmd_project_delete(project_name: &str) -> zigzag_core::error::Result<String> {
    let mut store = KdlProjectStore::new();
    delete_project_from_store(&mut store, project_name)
}

/// Kill a Zellij session by project+branch (zigzag session kill <project> <branch>).
///
/// For remote projects, delegates to the remote machine via SSH.
fn cmd_session_kill(project_name: &str, branch: &str) -> zigzag_core::error::Result<()> {
    let session_mgr = ZellijSessionManager {
        bin_path: resolve_bin_path(),
    };
    ...
    if let Some(proj) = &project {
        if let Some(host) = &proj.host {
            ...
            let session_name = zigzag_core::domain::Session::new(remote_name, branch).name;
    ...
    let session_name = zigzag_core::domain::Session::new(project_name, branch).name;
...
fn cmd_worktree_delete(
    ...
    if let Some(host) = project.host.as_deref() {
        if !confirmed {
            return Err(zigzag_core::error::ZError::Worktree(
        ...
        let remote_cmd = format!(
            "cd {} && zigzag worktree delete {} {} --confirm {}",
            remote::shell_quote(&project.path.display().to_string()),
            remote::shell_quote(remote_name),
            remote::shell_quote(branch),
            remote::shell_quote(branch),
    ...
    let target = worktrees
        ...
        .ok_or_else(|| {
            zigzag_core::error::ZError::Worktree(format!(
                "worktree '{}' not found for project '{}'",
                branch, project_name
            ))
        })?;

    let target_session_name = zigzag_core::domain::Session::new(project_name, branch).name;
    ...
    if has_active_session {
        ...
        .map_err(|e| zigzag_core::error::ZError::Session(format!("Failed to kill session: {e}")))?;
    ...
    // Clean metadata
    let metadata_store = crate::worktree_metadata_store::LocalWorktreeMetadataStore::default();
    let identity = zigzag_core::domain::WorktreeIdentity {
        host: None,
        project_root: project.path.clone(),
...
fn cmd_doctor(fix: bool, interactive: bool) -> zigzag_core::error::Result<String> {
                ...
                for project in &projects {
                    ...
                    if let Some(host) = project.host.as_deref() {
                            ...
                            .output()
                        {
                            ...
                            .unwrap_or_default();
                            let entries = zigzag_core::worktree_topology::assemble_worktree_entries(
                                project,
                                worktrees.clone(),
                                &sessions,
                                &[],
                            ...
                            for session in zigzag_core::worktree_topology::find_orphan_sessions(
                                &sessions, &worktrees,
                            ) {
    ...
    if interactive {
        diagnostics.push(String::new());
        diagnostics.push(
            " --interactive: use dashboard confirmations or explicit zigzag session/worktree commands for destructive fixes."
                .to_string(),
        );
    }
...
fn doctor_fix_metadata() -> zigzag_core::error::Result<String> {
    let metadata_store = worktree_metadata_store::LocalWorktreeMetadataStore::default();
...
fn cmd_logs(n: usize) -> zigzag_core::error::Result<()> {
    let logger = log::FileLogger::new();
    if n == 0 {
...
fn check_switch_deps() -> zigzag_core::error::Result<()> {
...
fn switch_bootstrap_log_message(args: &[String]) -> String {
    ...
    format!(
        "bootstrap pid={} ppid={} exe={} args={:?} ZIGZAG_SESSION_NAME={} ZELLIJ_SESSION_NAME={}",
        std::process::id(),
        parent_process_id(),
        exe,
        args,
...
fn build_sorted_switch_entries(
    ...
    metadata: Option<&zigzag_core::domain::WorktreeMetadataFile>,
    discovered: &[zigzag_core::domain::DiscoveredWorktree],
    global: &GlobalConfig,
    session_activity: &zigzag_core::activity::SessionActivity,
) -> Vec<zigzag_tui::SwitchSessionEntry> {
    let mut entries =
        build_switch_entries(sessions, metadata, discovered, global, session_activity);
    zigzag_tui::sort_switch_entries(&mut entries, &global.switcher.priority);
    entries
}

fn load_local_switch_entries(global: &GlobalConfig) -> Vec<zigzag_tui::SwitchSessionEntry> {
    let activity = activity_store::FileActivityStore::default().load_activity();
    ...
    let metadata = worktree_metadata_store::LocalWorktreeMetadataStore::default()
        .read_metadata()
        .ok();
    let sessions = list_all_zigzag_sessions_with_ages();
    build_sorted_switch_entries(sessions, metadata.as_ref(), &discovered, global, &activity)
}

fn switch_to_session_inside_zellij(session_name: &str) -> zigzag_core::error::Result<()> {
    let output = std::process::Command::new("zellij")
...
fn session_from_name(session_name: &str) -> zigzag_core::error::Result<Session> {
    let (project, branch) = parse_session_name(session_name).ok_or_else(|| {
        zigzag_core::error::ZError::Session(format!(
            "invalid zigzag session name {:?}: expected project:branch",
            session_name
        ))
    })?;
    Ok(Session {
...
fn attach_session_outside_zellij(session_name: &str) -> zigzag_core::error::Result<()> {
...
fn execute_switch_selection(
    ...
    selected: &str,
) -> zigzag_core::error::Result<SwitchExecution> {
...
///
/// Guard: must be run inside a Zellij session (`$ZIGZAG_SESSION_NAME` or `$ZELLIJ_SESSION_NAME` set).
/// Lists all Zigzag-managed sessions, presents a picker TUI, and if the user
/// selects a session runs `zellij action switch-session <name>`.
fn cmd_switch() -> zigzag_core::error::Result<()> {
    ...
    if let Some(session_name) = selected {
        ...
        match execute_switch_selection(Some(&current_session), &session_name)? {
            ...
            SwitchExecution::AttachOutsideZellij => {
                unreachable!("zigzag switch always has current session")
            }
        }
    } else {
    ...
    sessions
        ...
        .map(|(session_name, age)| {
            ...
            if let Some(metadata) = metadata {
                if let zigzag_core::domain::SessionAliasResolution::Unique(worktree) =
                    zigzag_core::domain::resolve_session_alias(&session_name, discovered)
                {
                    ...
                    notifications = metadata_notifications
                        .into_iter()
                        .map(|notification| zigzag_tui::SwitchNotification {
                    ...
                    activity = metadata
                        ...
                        .filter_map(|status| match status.state {
                            zigzag_core::domain::AgentActivityState::Waiting => {
                                Some(zigzag_tui::SwitchAgentActivity {
                                    tool: status.tool.clone(),
                                    state: zigzag_tui::SwitchAgentActivityState::Waiting,
                                    updated_at_ms: status.updated_at_ms,
                                    reason: status.reason.clone(),
                                })
                            }
                            zigzag_core::domain::AgentActivityState::Working => {
                                if zigzag_core::agent_activity::working_status_is_fresh(
                                    status, now_ms, ttl_ms,
                                ) {
                                    Some(zigzag_tui::SwitchAgentActivity {
                                        tool: status.tool.clone(),
                                        state: zigzag_tui::SwitchAgentActivityState::Working,
                                        updated_at_ms: status.updated_at_ms,
                                        reason: status.reason.clone(),
                                    })
                                } else {
                        ...
                        .max_by_key(|activity| match activity.state {
                            zigzag_tui::SwitchAgentActivityState::Waiting => {
                                (1u8, activity.updated_at_ms)
                            }
                            zigzag_tui::SwitchAgentActivityState::Working => {
...
fn cmd_logs_viewer() -> zigzag_core::error::Result<()> {
    let logger = log::FileLogger::new();
    let entries = logger.read_recent(500);
    let lines: Vec<String> = entries.iter().map(|e| e.format()).collect();
    zigzag_tui::run_log_viewer(lines).map_err(|e| zigzag_core::error::ZError::Io(e.to_string()))
}

/// Run the action picker inside a Zellij floating pane.
/// Reads `ZIGZAG_SESSION_NAME` with `ZELLIJ_SESSION_NAME` fallback to detect project/branch context,
/// resolves available actions, shows a picker, and executes the selection.
fn cmd_actions() -> zigzag_core::error::Result<()> {
    ...
    let action_preview = preview_source
        ...
        .map(|extra| {
            zigzag_core::action::ActionPreview::from_forge_data(
                extra.pr.as_ref(),
                Some(extra.ci),
                extra.review.as_ref(),
            )
    ...
    let env = zigzag_core::action::ActionEnv::for_session(
        project.name.clone(),
    ...
    let merged = zigzag_core::action::merge_actions(&[
        zigzag_core::action::builtin_actions(),
        global.actions.clone(),
        per_repo.actions.clone(),
    ]);

    let actions = zigzag_core::action::resolve_actions(&merged, &env).unwrap_or_default();
    ...
    if let Some(action) = selected {
        match &action.action {
            zigzag_core::action::ActionType::Run { command } => {
...
fn inject_claude_stop_hook(cwd: &std::path::Path) {
    ...
    let merged = zigzag_core::claude_hook::merge_stop_hook(
        existing,
...
fn cmd_notify(session: &str, message: &str, level: NotifyLevel) -> zigzag_core::error::Result<()> {
...
fn cmd_notify_event(event: NotifyEventCommand) -> zigzag_core::error::Result<()> {
    let target = resolve_notify_target(&event.session)?;
    match target {
        NotifyTarget::Local(identity) => {
            let global = load_global_config();
            let settings = zigzag_core::agent_activity::AgentActivitySettings::from_seconds(
...
fn apply_event_after_successful_migration<F>(
    migration: zigzag_core::error::Result<()>,
    apply: F,
) -> zigzag_core::error::Result<()>
where
    F: FnOnce() -> zigzag_core::error::Result<()>,
{
    migration?;
    apply()
}
...
enum NotifyTarget {
    Local(zigzag_core::domain::WorktreeIdentity),
    Remote {
        host: String,
        session_name: String,
        identity: zigzag_core::domain::WorktreeIdentity,
...
fn resolve_notify_target(session_name: &str) -> zigzag_core::error::Result<NotifyTarget> {
    let store = KdlProjectStore::new();
    for project in store.list_projects()? {
        ...
        return match zigzag_core::domain::resolve_session_alias(session_name, &discovered) {
            zigzag_core::domain::SessionAliasResolution::Unique(worktree) => {
            ...
            zigzag_core::domain::SessionAliasResolution::Ambiguous(_) => {
                Err(zigzag_core::error::ZError::Session(format!(
...
impl NotifyEventCommand {
    ...
    fn to_remote_command(&self, session: &str) -> String {
        let mut command = format!(
            "zigzag notify --event {} --tool {} --session {}",
...
fn resolve_notify_event_args(
    ...
    let session = session
        ...
        .ok_or_else(|| {
            "no session specified and neither ZIGZAG_SESSION_NAME nor ZELLIJ_SESSION_NAME is set"
                .to_string()
        })?;
...
/// (`ZIGZAG_SESSION_NAME` with `ZELLIJ_SESSION_NAME` fallback), if any.
///
...
fn resolve_notify_args(
    ...
    let (session, message) = match positional.len() {
        ...
        _ => {
            return Err(
                "usage: zigzag notify [session] <message> [--level info|warning|error]".to_string(),
            );
        }
    };
...
mod tests {
    ...
    fn clear_session_env() {
        std::env::remove_var("ZIGZAG_SESSION_NAME");
    ...
    fn test_identity(project_name: &str) -> zigzag_core::domain::WorktreeIdentity {
        zigzag_core::domain::WorktreeIdentity {
            host: None,
    ...
    fn test_discovered_worktree(project_name: &str) -> zigzag_core::domain::DiscoveredWorktree {
        zigzag_core::domain::DiscoveredWorktree {
    ...
    impl ProjectStoreWriter for RecordingProjectStore {
        ...
        fn remove_project(&mut self, name: &str) -> zigzag_core::error::Result<()> {
            self.removed.push(name.to_string());
            Ok(())
        }

        fn swap_projects(&mut self, _a: usize, _b: usize) -> zigzag_core::error::Result<()> {
            Ok(())
        }
    }
    ...
    fn build_switch_entries_attaches_metadata_notifications_and_waiting_activity() {
        ...
        let metadata = zigzag_core::domain::WorktreeMetadataFile {
            version: 2,
            worktrees: vec![],
            notifications: vec![zigzag_core::domain::NotificationRecord {
                id: "n1".to_string(),
                target: test_identity(&project_name),
                level: zigzag_core::domain::NotifyLevel::Warning,
            ...
            llm_status: vec![zigzag_core::domain::AgentActivityStatus {
                target: test_identity(&project_name),
                tool: "opencode".to_string(),
                state: zigzag_core::domain::AgentActivityState::Waiting,
                updated_at_ms: 1_500,
        ...
        let entries = build_switch_entries(
            ...
            &global,
            &zigzag_core::activity::SessionActivity::new(),
        );
        ...
        assert_eq!(
            ...
            Some(zigzag_tui::SwitchAgentActivityState::Waiting)
        );
    }
    ...
    fn build_sorted_switch_entries_applies_switcher_priority() {
        ...
        let mut activity = zigzag_core::activity::SessionActivity::new();
        activity.insert(first_session.clone(), 10);
        activity.insert(second_session.clone(), 20);
        let mut global = GlobalConfig::default();
        global.switcher.priority = vec![zigzag_core::config::SwitcherPriorityCriterion::Recent];
    ...
    fn build_switch_entries_drops_stale_working_activity() {
        ...
        let metadata = zigzag_core::domain::WorktreeMetadataFile {
            ...
            llm_status: vec![zigzag_core::domain::AgentActivityStatus {
                target: test_identity(&project_name),
                tool: "opencode".to_string(),
                state: zigzag_core::domain::AgentActivityState::Working,
                updated_at_ms: 1,
                reason: None,
    ...
    fn build_switch_entries_counts_metadata_notifications() {
        ...
        let metadata = zigzag_core::domain::WorktreeMetadataFile {
            ...
            notifications: vec![zigzag_core::domain::NotificationRecord {
                id: "n1".to_string(),
                target: test_identity(&project_name),
                level: zigzag_core::domain::NotifyLevel::Info,
    ...
    fn notify_event_apply_is_blocked_when_migration_fails() {
        ...
        let result = apply_event_after_successful_migration(
            Err(zigzag_core::error::ZError::MetadataCorrupt(
                "bad json".to_string(),
            )),
            || {
                called = true;
    ...
    fn delete_project_from_store_rejects_empty_name() {
        ...
        assert!(matches!(err, zigzag_core::error::ZError::ConfigParse(_)));
    ...
    fn resolve_notify_args_one_positional_no_env_returns_error() {
        ...
        assert!(
            err.contains("ZIGZAG_SESSION_NAME"),
    ...
    fn resolve_required_session_env_mentions_both_env_vars_when_missing() {
        ...
        assert!(
            err.contains("ZIGZAG_SESSION_NAME"),
            "error should mention ZIGZAG_SESSION_NAME: {err}"
        );
        assert!(
    ...
    fn format_run_status_includes_header() {
        let run = zigzag_autopilot::state::WorkflowRun::new("pr-ci-fix", "myapp", "monitor-ci");
        let runs = vec![&run];
    ...
    fn format_run_status_shows_completed_status() {
        use zigzag_autopilot::state::WorkflowStatus;
        let mut run = zigzag_autopilot::state::WorkflowRun::new("pr-ci-fix", "myapp", "monitor-ci");
    ...
    fn format_run_status_shows_failed_status() {
        let mut run = zigzag_autopilot::state::WorkflowRun::new("wf1", "proj", "step1");
    ...
    fn format_workflow_list_single_with_no_description() {
        use zigzag_autopilot::dsl::{AutopilotWorkflow, Trigger};
    ...
    fn format_run_status_multiple_runs() {
        let run1 = zigzag_autopilot::state::WorkflowRun::new("wf1", "proj-a", "step1");
        let run2 = zigzag_autopilot::state::WorkflowRun::new("wf2", "proj-b", "step2");
    ...
    fn edit_per_repo_config_creates_config_dir_and_template() {
        ...
        // Neither .config/ nor .config/zigzag.kdl exist yet.
        ...
        let config_file = project_path.join(".config").join("zigzag.kdl");
        assert!(config_file.exists(), ".config/zigzag.kdl should be created");
    ...
    fn edit_per_repo_config_does_not_overwrite_existing_file() {
        ...
        let config_file = config_dir.join("zigzag.kdl");
...
/// Dispatch `zigzag autopilot [subcommand] [args]`.
fn cmd_autopilot_dispatch(sub: Option<&str>, args: &[String]) -> zigzag_core::error::Result<()> {
    match sub {
        None | Some("help") => {
            println!("usage: zigzag autopilot <subcommand>");
            println!();
            println!("subcommands:");
        ...
        Some("run") => {
            ...
            if project.is_empty() || workflow.is_empty() {
                return Err(zigzag_core::error::ZError::Io(
                    "usage: zigzag autopilot run <project> <workflow>".to_string(),
                ));
            }
            cmd_autopilot_run(project, workflow)
        }
        Some(unknown) => Err(zigzag_core::error::ZError::Io(format!(
            "unknown autopilot subcommand: {:?}\nusage: zigzag autopilot [list|status|prune|run]",
            unknown
        ))),
    }
}
...
) -> zigzag_core::error::Result<Vec<AutopilotWorkflow>> {
    let mut all_workflows: Vec<AutopilotWorkflow> = builtin_workflows()
        .map_err(|e| zigzag_core::error::ZError::Io(format!("load built-in workflows: {e}")))?;

    if let Some(path) = project_path {
        let repo_config_path = path.join(".config").join("zigzag.kdl");
...
fn cmd_autopilot_run(project_name: &str, workflow_name: &str) -> zigzag_core::error::Result<()> {
    let store = KdlProjectStore::new();
    let project = store.get_project(project_name)?;
    ...
    let workflow = workflows
        ...
        .ok_or_else(|| {
            zigzag_core::error::ZError::ConfigParse(format!(
...
pub fn cmd_autopilot_status(project_filter: Option<&str>) -> zigzag_core::error::Result<()> {
...
pub fn cmd_autopilot_prune(project_filter: Option<&str>) -> zigzag_core::error::Result<()> {
    let state_dir = autopilot_state_dir();
...
fn cmd_list() -> zigzag_core::error::Result<()> {
    let store = KdlProjectStore::new();
    let session_mgr = ZellijSessionManager {
        bin_path: resolve_bin_path(),
    };
