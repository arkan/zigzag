...
use zigzag_core::domain::{DiscoveredWorktree, GitSafetyStatus, Worktree, WorktreeIdentity};
use zigzag_core::error::{Result, ZError};
use zigzag_core::traits::WorktreeManager;
...
pub fn find_worktree_path_for_branch(worktrees: &[Worktree], branch: &str) -> Option<PathBuf> {
    worktrees
        .iter()
        .find(|worktree| zigzag_core::domain::sanitize_branch_name(&worktree.branch) == branch)
