...
use zigzag_core::config::{parse_projects_kdl, swap_project_nodes};
use zigzag_core::domain::Project;
use zigzag_core::error::{Result, ZError};
use zigzag_core::traits::{ProjectStore, ProjectStoreWriter};

/// Returns the path to `~/.config/zigzag/projects.kdl`.
pub fn projects_kdl_path() -> PathBuf {
    crate::paths::projects_path()
}

/// A `ProjectStore` that reads from `~/.config/zigzag/projects.kdl`.
...
fn format_project_kdl(project: &Project) -> String {
    ...
    if let Some(transport) = &project.transport {
        let val = match transport {
            zigzag_core::domain::Transport::Ssh => "ssh",
            zigzag_core::domain::Transport::Mosh => "mosh",
        };
...
"#;
    ...
    fn kdl_project_store_missing_file_returns_empty() {
        let path = PathBuf::from("/tmp/zigzag_nonexistent_test_file_xyz.kdl");
...
"#;
    ...
    fn format_project_kdl_writes_transport_mosh() {
        let project = Project {
            ...
            path: std::path::PathBuf::from("/code/app"),
            host: Some("vps".to_string()),
            transport: Some(zigzag_core::domain::Transport::Mosh),
        };
        let kdl = format_project_kdl(&project);
        assert!(
    ...
    fn format_project_kdl_writes_transport_ssh() {
        let project = Project {
            ...
            path: std::path::PathBuf::from("/code/app"),
            host: None,
            transport: Some(zigzag_core::domain::Transport::Ssh),
        };
        let kdl = format_project_kdl(&project);
        assert!(
