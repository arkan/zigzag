...
use zigzag_core::domain::Session;
...
pub fn write_remote_zigzag_config_file_atomic(
    ...
    let cmd = format!(
        "set -e; dir=\"${{XDG_CONFIG_HOME:-$HOME/.config}}/zigzag\"; mkdir -p \"$dir\"; file=\"$dir/{0}\"; lock=\"$dir/{0}.lock\"; tmp=\"$dir/{0}.tmp.$$\"; i=0; while ! mkdir \"$lock\" 2>/dev/null; do i=$((i+1)); if [ $i -ge 10 ]; then echo 'metadata lock unavailable' >&2; exit 4; fi; sleep 0.05; done; trap 'rm -rf \"$lock\" \"$tmp\"' EXIT; cat > \"$tmp\"; mv \"$tmp\" \"$file\"; rm -rf \"$lock\"; trap - EXIT",
        safe_filename
    );
