...
//!
//! Stores worktree metadata at `~/.config/zigzag/worktree-metadata.json` with
...
//! overwritten.
//!
//! Migration helpers read legacy `~/.config/zigzag/session-activity.json` and
//! `/tmp/zigzag/notifications/` and attempt to resolve old session names against
...
use zigzag_core::domain::{
    DiscoveredWorktree, NotificationRecord, NotifyLevel, UnattachedActivity,
    UnattachedNotification, WorktreeIdentity, WorktreeMetadataFile, WorktreeMetadataRecord,
};
use zigzag_core::error::{Result, ZError};
use zigzag_core::traits::WorktreeMetadataStore;

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------

const METADATA_FILENAME: &str = crate::paths::WORKTREE_METADATA_FILE;
...
const LEGACY_NOTIFICATIONS_DIR: &str = crate::paths::NOTIFICATIONS_DIR;
const CURRENT_VERSION: u32 = zigzag_core::agent_activity::WORKTREE_METADATA_VERSION;
...
/// `~/.config/zigzag/worktree-metadata.json` with lock-file protection.
#[derive(Debug, Clone)]
pub struct LocalWorktreeMetadataStore {
    config_dir: PathBuf,
}

/// SSH-backed adapter for a remote host's `~/.config/zigzag/worktree-metadata.json`.
#[derive(Debug, Clone)]
...
impl RemoteWorktreeMetadataStore {
    ...
    /// Clear notifications attached to a remote Worktree identity after successful entry.
    pub fn clear_notifications(&self, identity: &WorktreeIdentity) -> Result<()> {
        let mut file = self.read_metadata()?;
...
impl LocalWorktreeMetadataStore {
    /// Create a store rooted at `config_dir` (the `~/.config/zigzag/` directory).
    ...
    /// Final idempotent drain of legacy `/tmp/zigzag/notifications/` into metadata.
    ///
    ...
    ///
    /// If `legacy_notifications_path` is `None`, defaults to `/tmp/zigzag/notifications/`.
    pub fn drain_legacy_notifications(
        ...
        for (session_name, notifications) in &legacy_notifications {
            for (file_id, level, message, created_at) in notifications {
                ...
                if metadata.migrated_legacy_ids.contains(&dedup_key) {
                    files_to_cleanup.push(notifications_path.join(session_name).join(file_id));
                    continue; // already migrated
                }

                let resolution =
                    zigzag_core::domain::resolve_session_alias(session_name, discovered_worktrees);
                match resolution {
                    zigzag_core::domain::SessionAliasResolution::Unique(wt) => {
                        ensure_worktree_record(&mut metadata.worktrees, &wt);
                        metadata
                            .notifications
                            .push(zigzag_core::domain::NotificationRecord {
                                id: file_id.clone(),
        ...
        // Delete only files that were successfully read and represented in metadata.
        if !files_to_cleanup.is_empty() && notifications_path.exists() {
            ...
            match resolution {
                ...
                zigzag_core::domain::SessionAliasResolution::None => {
                    ...
                    report.diagnostics.push(format!(
                        "unresolved session name (activity): {session_name}"
                    ));
                }
            }
        }
    ...
    fn read_legacy_notifications(
        ...
        for entry in entries.flatten() {
            ...
            for file_entry in fs::read_dir(&session_dir).into_iter().flatten() {
                ...
                let id = file_entry.file_name().to_string_lossy().to_string();
...
impl LocalWorktreeMetadataStore {
    ...
    pub fn remove_worktree(&self, identity: &WorktreeIdentity) -> Result<()> {
        self.update_metadata(|file| {
            file.version = CURRENT_VERSION;
            file.worktrees
                .retain(|record| !record_matches_identity(record, identity));
            file.notifications
                .retain(|notification| notification.target != *identity);
    ...
    pub fn apply_agent_activity(
        ...
        target: WorktreeIdentity,
        tool: &str,
        event: zigzag_core::agent_activity::AgentActivityEvent,
        reason: Option<String>,
        settings: zigzag_core::agent_activity::AgentActivitySettings,
    ) -> Result<bool> {
        let now = unix_now_ms();
        let notification_id = format!("{}-{}", now, std::process::id());
        self.update_metadata_if_changed(|file| {
            zigzag_core::agent_activity::apply_agent_activity_update(
                file,
                zigzag_core::agent_activity::AgentActivityUpdate {
                    target,
                    tool: tool.to_string(),
                    event,
                    reason,
    ...
    pub fn notification_session_aliases(
        &self,
        discovered: Option<&[zigzag_core::domain::DiscoveredWorktree]>,
    ) -> Result<std::collections::HashSet<String>> {
        let file = self.read_metadata()?;
        Ok(notification_aliases_from_metadata(&file, discovered))
    }
}

pub(crate) fn notification_aliases_from_metadata(
    file: &WorktreeMetadataFile,
    discovered: Option<&[zigzag_core::domain::DiscoveredWorktree]>,
) -> std::collections::HashSet<String> {
    let mut aliases = std::collections::HashSet::new();
    for notification in &file.notifications {
        if let Some(discovered) = discovered {
            if let Some(wt) = discovered
                .iter()
                .find(|wt| wt.identity == notification.target)
            {
                if let Some(branch) = &wt.branch {
                    aliases.insert(zigzag_core::domain::derive_session_name(
                        &wt.project_name,
                        branch,
                    ));
                    continue;
...
impl WorktreeMetadataStore for RemoteWorktreeMetadataStore {
    fn read_metadata(&self) -> Result<WorktreeMetadataFile> {
        match crate::remote::read_remote_zigzag_config_file(&self.host, METADATA_FILENAME)? {
    ...
    fn write_metadata(&self, data: &WorktreeMetadataFile) -> Result<()> {
        ...
        crate::remote::write_remote_zigzag_config_file_atomic(&self.host, METADATA_FILENAME, &bytes)
    }
}
...
mod tests {
    ...
    fn migration_preserves_unattached_notifications() {
        ...
        fs::write(session_dir.join("123_0"), "warning\nLegacy notification").unwrap();
    ...
    fn remote_store_normalizes_host_for_local_view_and_strips_for_remote_file() {
        ...
        assert_eq!(
            local_view.worktrees[0].host.as_deref(),
            Some("remote.example")
        );
        assert_eq!(
            local_view.notifications[0].target.host.as_deref(),
            Some("remote.example")
        );
    ...
    fn migration_activity_picks_most_recent_timestamp() {
        ...
        let worktrees = vec![make_discovered_wt("myapp", "/repo", "/repo", Some("main"))];
    ...
    #[test]
    fn drain_preserves_unread_legacy_files_when_cleaning_processed_files() {
        let (store, dir) = temp_store();
        let notif_base = test_dir();

        write_legacy_notification(&notif_base, "myapp:main", "1710000000_0", "info\nHello");
        fs::create_dir_all(notif_base.join("myapp:main").join("unread-entry")).unwrap();
        let worktrees = vec![make_discovered_wt("myapp", "/repo", "/repo", Some("main"))];
        ...
        assert_eq!(report.migrated_notifications, 1);
        assert!(!notif_base.join("myapp:main").join("1710000000_0").exists());
        assert!(notif_base.join("myapp:main").join("unread-entry").exists());
    ...
    #[test]
    fn notification_aliases_can_use_discovered_worktree_without_metadata_record() {
        let identity = WorktreeIdentity {
            host: None,
            project_root: PathBuf::from("/repo"),
            worktree_path: PathBuf::from("/repo/.worktrees/feat-login"),
        };
        let metadata = WorktreeMetadataFile {
            version: CURRENT_VERSION,
            worktrees: vec![],
            notifications: vec![NotificationRecord {
                id: "n1".to_string(),
        ...
        let aliases = notification_aliases_from_metadata(&metadata, Some(&discovered));

        assert!(aliases.contains("myapp:feat-login"));
    }
