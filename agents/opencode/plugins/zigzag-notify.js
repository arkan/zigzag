// OpenCode plugin: publish OpenCode activity to Zigzag.
//
// Emits structured `zigzag notify --event` updates when OpenCode works, waits for
// user intervention, or errors.
// Resolves the target session from ZIGZAG_SESSION_NAME (set by Zellij layout env block)
...
function sessionName() {
  return process.env.ZIGZAG_SESSION_NAME || process.env.ZELLIJ_SESSION_NAME || "";
}
...
async function notifyActivity($, kind, eventOrInput, eventName) {
  ...
  if (!session) return; // no-op outside a Zigzag-managed session
  if (!shouldSend(kind, eventOrInput)) return;

  try {
    await $`zigzag notify --event ${eventName} --tool ${TOOL} --session ${session}`;
  } catch {
...
async function notifyWaiting($, kind, eventOrInput, reason, text, level) {
  ...
  try {
    await $`zigzag notify --event llm.waiting --tool ${TOOL} --session ${session} --reason ${reason} --message ${text} --level ${level}`;
  } catch {
    // Silently ignore failures — activity reporting is best-effort.
  }
}

async function ZigzagNotifyPlugin({ $ }) {
  return {
    event: async ({ event }) => {
...
export { ZigzagNotifyPlugin };
export default ZigzagNotifyPlugin;
