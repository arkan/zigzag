...
pub(crate) fn record_attach_at(path: &Path, session: &str, now_secs: u64) -> Result<()> {
    let mut activity = load_activity_from(path);
...
/// or malformed.
pub(crate) fn load_activity_from(path: &Path) -> HashMap<String, u64> {
...
/// missing.
pub(crate) fn remove_entry_at(path: &Path, session: &str) -> Result<()> {
    let mut activity = load_activity_from(path);
...
fn write_activity(path: &Path, activity: &HashMap<String, u64>) -> Result<()> {
    ...
    fs::rename(&tmp, path).map_err(|e| ZError::Io(e.to_string()))?;
    Ok(())
}
...
pub fn sort_sessions_by_recent_attach(
...
pub fn sort_by_recent_attach<T, F>(
    items: &mut [T],
    activity: &HashMap<String, u64>,
    name_of: F,
) where
